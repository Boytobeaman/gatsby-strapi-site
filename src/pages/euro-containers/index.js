...
// need to change
import ThisCatDesc from '../../components/productDesc/StackingCrate'
...
const JSONbuildtime = ({data}) => {
  ...
  cat_link= `/${cat_link}/`
  // need to change
  let cat_text = menu.stackingCrate.text
...
export const query = graphql`
  query{
    strapidata: allStrapiWebsiteseometa(filter: {domain: {name: {eq: "bulk-containers.com"}}, product_identify_cat: {eq: "stacking crate"}}) {
      edges {
        node {
          id
          strapiId
    ...
    bannerDesktop: file(relativePath: {eq: "euro-stacking-container-banner.jpg"}) {
      childImageSharp {
        fluid {
          ...GatsbyImageSharpFluid
        }
    ...
    # need to change the file path
    bannerPhone: file(relativePath: {eq: "euro-stacking-container-banner-phone.jpg"}) {
      childImageSharp {
        fluid {
          ...GatsbyImageSharpFluid
        }
